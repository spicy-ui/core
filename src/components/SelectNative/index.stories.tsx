...
import { SelectNative, Stack } from '..';
...
export const Simple = (props: any) => (
  <SelectNative {...props} onChange={({ target }) => action('onChange')(target.value)}>
    <Options />
  </SelectNative>
);
...
export const AllSpaces = (props: any) => (
  <Stack spacing="4">
    {selectSpaces.map((space, idx) => (
      <SelectNative key={uid(space, idx)} {...props} space={space} placeholder={space}>
        <Options />
      </SelectNative>
    ))}
  </Stack>
...
export const AllVariants = (props: any) => (
  <Stack spacing="4">
    {selectVariants.map((variant, idx) => (
      <SelectNative key={uid(variant, idx)} {...props} variant={variant} placeholder={variant}>
        <Options />
      </SelectNative>
    ))}
  </Stack>
