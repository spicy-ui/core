...
export const Drawer: ComponentTheme = {
  baseStyle: (props: any) => ({
    ...
    width: ['top', 'bottom'].includes(props.anchor) ? '100vw' : '100%',
    height: ['left', 'right'].includes(props.anchor) ? '100vh' : '100%',
    maxWidth: ['top', 'bottom'].includes(props.anchor) ? '100vw' : `calc(100vw - ${props.theme.space['12']})`,
    maxHeight: ['left', 'right'].includes(props.anchor) ? '100vh' : `calc(100vh - ${props.theme.space['12']})`,
